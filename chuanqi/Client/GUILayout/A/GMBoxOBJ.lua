...
GMBoxOBJ.cfg = {}
local _cfg = SL:Require("GUILayout/A/cfg_gm_box",true)
for _, v in ipairs(_cfg) do
    GMBoxOBJ.cfg[v.page] = GMBoxOBJ.cfg[v.page] or {title = v.pageName}
    GMBoxOBJ.cfg[v.page][#GMBoxOBJ.cfg[v.page] + 1] = {
        page = v.page,
        index = v.index,
        title = v.title,
        Command = v.Command,
...
function GMBoxOBJ:main()
    ...
    GUI:LoadExport(parent, "A/GMBoxUI",function ()
        ...
        local handle = GUI:ItemBox_Create(parent, "ItemBox_Create", 0, 0, "res/public/1900000663.png", 100, "*")
        GUI:ItemBox_UpdateBoxData(handle, 100)
        ...
        --发送命令
        GUI:addOnClickEvent(self.ui.Button_send, function(sender)
...
function GMBoxOBJ:createInput()
    ...
    GUI:setTouchEnabled(Input, true)
    GUI:TextInput_setInputMode(Input,6)
    GUI:TextInput_closeInput(self.cacheInput)
    GUI:TextInput_addOnEvent(Input, function(sender)
        SL:Print("进入TextInput_addOnEvent")
        self.cmdIndex = nil
        self.cacheCmd = nil
        GUI:TextInput_setString(sender,"")

        if not next(self.cmdList) then return end
    ...
    --缓存命令按钮
    GUI:addOnClickEvent(self.ui.Button_input_5, function(sender)

        SL:Print("TextInput_closeInput测试","/ɑ:/ /ʌ/ /ɔ:/ /ɒ/ /ɜ:/ /ə/ /i:/ /ɪ/ /u:/ /ʊ/ /e/ /æ/ /eɪ/ /aɪ/ /ɔɪ/ /ɪə/ /eə/ /ʊə/ /əʊ/ /aʊ/")
        GUI:TextInput_closeInput(self.cacheInput)
        -- if not next(self.cmdList) then return end
