...
-- 创建满足条件的红点函数
function creatRedDot(parent, redDot_ID, x_coord, y_coord)
  ssrPrint("888", parent, "999", redDot_ID)
  -- 检测条件是否满足
  if SL:GetMetaValue("CHECK_REDPOINT_ID", redDot_ID) then
    -- 获取父控件的大小 并 设置红点控件的坐标
    ssrPrint("222", redDot_ID)
    local parent_size = GUI:getContentSize(parent)
    local X_coord = x_coord or parent_size.width - 18
    local Y_coord = y_coord or parent_size.height - 18
    -- 创建红点图片
    local reddot = GUI:Image_Create(parent, "reddot" .. redDot_ID, X_coord, Y_coord, "res/public/btn_npcfh_04.png")
    -- 加入红点控件到红点控件总表中
    redDots[redDot_ID] = reddot
  end
end

-- 移除不满足条件的红点控件
function removeRedDot()
  for key, value in pairs(redDots) do
    if not SL:GetMetaValue("CHECK_REDPOINT_ID", key) then
      pcall(function()
        GUI:removeFromParent(value)
      end)
      redDots[key] = nil
    end
  end
...
-- 注册监听事件，当服务器下发的变量改变时调用
SL:RegisterLUAEvent(LUA_EVENT_SERVER_VALUE_CHANGE, "removeRedDot", removeRedDot)
