...
--道具合成
function HechengNPC.RequestHecheng(actor,idx)
    idx = tonumber(idx) 
    ...
    if name then
        ...
        if cfg.npcid then
            Message.sendmsg(actor, ssrNetMsgCfg.HechengNPC_HechengResponse)
        end
        return
    end
...
Message.RegisterNetMsg(ssrNetMsgCfg.HechengNPC, HechengNPC)
