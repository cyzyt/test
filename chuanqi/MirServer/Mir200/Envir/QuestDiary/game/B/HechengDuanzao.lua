...
--道具合成
function HechengDuanzao.RequestHecheng(actor,idx)
    idx = tonumber(idx)
    ...
    if cfg.npcid ~= 262 then return end
    ...
    if name then
        ...
        if cfg.npcid then
            Message.sendmsg(actor, ssrNetMsgCfg.HechengDuanzao_HechengResponse)
        end
        return
    end
...
Message.RegisterNetMsg(ssrNetMsgCfg.HechengDuanzao, HechengDuanzao)

return HechengDuanzao